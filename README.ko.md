...
코드 없이도 됩니다. Info.plist에 키를 적으면 SDK가 스스로 시작합니다. 네이티브 크래시
훅은 라이브러리 초기화 시점, 즉 `main`보다 먼저 들어가므로 앱 자체 시작 중의 크래시도
이미 잡히고, 나머지는 메인 큐의 첫 턴에 시작됩니다. `Atlas.start`를 함께 호출해도 잃는
것은 없습니다. 두 번째 시작은 아무 일도 하지 않습니다. `AtlasBaseURL`은 서버를 바꿉니다.
...
| 잡히지 않은 C++ 예외. | 런타임이 abort할 때 날아가던 예외의 타입을 핸들러에서 읽고 다음 실행 때 디맹글합니다. 이슈는 또 하나의 SIGABRT가 아니라 `std::runtime_error`가 되고, `what()`이 메시지에 실립니다. |
| AppKit이 메인 스레드에서 삼킨 예외(macOS). | `-[NSApplication reportException:]`을 훅합니다. 예외 자신의 스택과 함께 오류로 기록한 뒤 그대로 넘깁니다. Info.plist의 `AtlasCrashOnNSException`은 이를 치명적으로 바꾸는데, 이는 AppKit 자체의 스위치이며 앱이 선택할 일입니다. |
| 보호된 파일 디스크립터나 mach 포트의 오용. | mach 서버의 `EXC_GUARD`. 가드 종류로 이름 붙습니다. |
...
라이브러리의 프레임은 다음 실행 때 같은 라이브러리에서 이름을 찾아 붙이므로, dSYM이
닿을 수 없는 자리에서도 `abort`와 `objc_msgSend`로 읽힙니다. 시작 후 5초 안에 난
크래시는 다음 실행에서 가장 먼저 전송됩니다.

SDK가 스스로 남기는 브레드크럼도 있습니다. 모두 노티피케이션에서 오며 권한이 필요
없습니다. 앱의 active, inactive, background, foreground 전환, 메모리 경고와 커널의
메모리 압박 단계(OOM kill의 사유에도 붙습니다), 방향, 키보드, 스크린샷, 씬과 윈도우
변화, 발열과 저전력 변화, 시간대와 시계 변경. macOS에서는 앱의 숨김과 표시, 윈도우
변화입니다.
